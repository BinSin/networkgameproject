...
public class Player extends Mob {
	...
	// private int colour = Colours.get(-1, 111, 200, 543);
	// 케릭터 크기
	private int scale = 1;
	...
	public Player(Level level, int x, int y, int colour, InputHandler input, String username) {
		super(level, "Player", x, y, colour, 3);
		this.input = input;
	...
	public void tick() {
		...
		if (xa != 0 || ya != 0) {
			...
			Packet02Move packet = new Packet02Move(this.getUsername(), this.x, this.y, this.numSteps, this.isMoving,
					this.movingDir, this.whoCatch, this.whoRelease, this.speed);
		...
		else if (isDamage && level.getTile(this.x >> 3, this.y >> 3).getId() != 18) {
			isDamage = false;
		}

		else if (level.getTile(this.x >> 3, this.y >> 3).getId() == 3) {
			isSwimming = true;
		}

		else if (level.getTile(this.x >> 3, this.y >> 3).getId() == 18) {
			isDamage = true;
		}
