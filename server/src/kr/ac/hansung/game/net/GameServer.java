...
public class GameServer extends Thread {
	...
	private void parsePacket(byte[] data, InetAddress address, int port) {
		...
		case LOGIN:
			...
			ChatVisual.chatScreen.append(((Packet00Login) packet).getUsername()+ " 님이 접속하였습니다."+"\n");
			// 케릭터 설정 부분에서 이곳 주의!
		...
		case DISCONNECT:
			...
			ChatVisual.chatScreen.append(((Packet01Disconnect)packet).getUsername()+ " 님이 종료하였습니다."+"\n");
		...
		case MOVE:
			...
			if (this.connectedPlayers.size() > 0) {
				...
				for (int i = 1; i < this.connectedPlayers.size(); i++) {
					...
					if (num == 1) {
						((Packet02Move) packet).setSpeed(10);
						GameEvents.check = true;
						sendDataToAllClients(((Packet02Move) packet).getData());
						break;
					}
					...
					if (((thisX > sullraeX - 5 && thisX < sullraeX + 5)
						...
						if (connectedPlayers.get(i).getSpeed() != 0) {
							...
							ChatVisual.chatScreen.append(connectedPlayers.get(i).getUsername() + "을 잡았습니다.\n");
						...
						if (connectedPlayers.get(i).getSpeed() > 0) {
							if (((freezeX > thisX - 5 && freezeX < thisX + 5)
								...
								if (connectedPlayers.get(j).getSpeed() == 0) {
									...
									ChatVisual.chatScreen.append(connectedPlayers.get(i).getUsername() + "가 "
											+ connectedPlayers.get(j).getUsername() + "를 풀어줬습니다.\n");
